...
% Particle size
radius =1/1.6; %In units of medium wavelength. 
...
N_theta=100; 
N_phi=100;% Make this divisible by 4, so that each quadrant includes the same number of points.
...
A_force_mat=T_E(Ap2,:)'*diag(PZ(Ap2))*T0+T0'*diag(PZ(Ap2))*T_E(Ap2,:)+T_E'*diag(PZ)*T_E;
...
if(Spring_kx(Iteration_num)>0)% Only apply ending condition if the force is negative
...
for num=1:length(x)
    ...
    %Gaussian trap
    phase_XY=KZ*x(num); % don't include n_medium because x is already given in medium wavelengths 
