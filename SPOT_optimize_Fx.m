...
%lateral force magnitude when applied at the back-focal plane.  
%
%Note, force is optimized at x=0, but to provide a more useful comparison
...
compute_Eigenmode=0;% A logical switch. If true, the Eigenmode solution will be computed and compared to phase-only optima.

% Particle size
radius =3/1.6; %In units of medium wavelength. 
...
% Angular basis: How many points along theta and phi do we use? 
% N_theta=80; 
% N_phi=80;% Make this divisible by 4, so that each quadrant includes the same number of points.
...
% n_particle = 2.3;% Titania
% n_particle = 1.78;% Amorphous Titania
...
% Step 1: Calculate T matrix and transform it to the E(theta,phi) basis.
...
% Longitudinal optical momentum
KZ=2*pi*cos(theta(Aperture));

% Now, include field amplitude
D_MX=-diag(E1(Ap2)')*A_force_mat*diag(E1(Ap2)); %use minus sign so it is still a maximization problem
...
phase_opt=zeros(size(theta(Aperture)));

ForceX=kx_mat(1);% Track changes in spring constant with iteration number.
...
for num=1:length(KX)
    % No symmetry
    phase_opt(num)=angle(D_MX_small(num,:)*(exp(1i*[phase_opt])));
...
ForceX(Iteration_num+1)=real((exp(1i*[phase_opt]))'*D_MX_small*(exp(1i*[phase_opt])));

if(ForceX(Iteration_num)>0)% Only apply ending condition if the force is negative
if(ForceX(Iteration_num+1)/ForceX(Iteration_num)<1.01) %Ending criteria
    %End loop
    run_optimization=0;
end
end
...
figure(1)
% xmax=x(Fxopt_mat==max(Fxopt_mat));
xneg=find(x>0&Fxopt_mat<0,1);
x_eq=(x(xneg)*Fxopt_mat(xneg-1)-x(xneg-1)*Fxopt_mat(xneg))/(Fxopt_mat(xneg-1)-Fxopt_mat(xneg));

plot(x,Fx0_mat,x-x_eq,Fxopt_mat)
% 
...
Z_opt = griddata(KX,KY,exp(1i*(phase_opt+KX*x_eq)),KXi,KYi);
% 
...
%     
%     figure(3)
%     Z_eig = griddata(KX,KY,Opt_Eig,KXi,KYi);
%     imagesc(angle(Z_eig))
