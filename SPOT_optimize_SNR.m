...
kx_norm=1;%n_medium^2/(wavelength*3e8);% Normalize trap stiffness to mN/m/W. 
...
Ap_cond=[theta(:)<asin(NA_condenser/n_medium);theta(:)<asin(NA_condenser/n_medium)];
...
PX=sign(cos([phi(:);phi(:)])).*Ap_cond.*[dA;dA];  
...
% Example signal calculation using M_mat;  Particle is displaced
...
figure(1)
subplot(3,1,2)
plot(x,Fx0_mat,x,Fx_OTT,x,Fx_OTT1) % Gaussian trap, calculated 3 ways. Agreement indicates that M_mat is calculated correctly.
figure(1)
subplot(3,1,3)
...
% dx_min=abs(wavelength/(n_medium*GX *sqrt(N_photflux)))
