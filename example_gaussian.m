...
diam_microns = radius * 0.78 * 2 / n_medium;
%
%     if Nmax < 12
%         Nmax = 12;
%     end
...
w0 = lg_mode_w0( lgmode, beam_angle );
...
[n,m,a0,b0] = bsc_pointmatch_farfield(Nmax,1,[ lgmode w0 1 polarisation 90 beam_offset ]);
[a,b,n,m] = make_beam_vector(a0,b0,n,m);
% load('ab_Hologram.mat');
...
% Calculation of trap stiffness
% Incl=abs(r)<0.2;
% 
% Gx=polyfit(r(Incl),fr(Incl),1);
% kx=-Gx(1)*n_medium^2/(1064e-9*3e8); %Normalization into N/m/W units
% 
% 
% Incl=abs(z-zeq)<0.2;
% Gz=polyfit(z(Incl),fz(Incl),1);
% kz=-Gz(1)*n_medium^2/(1064e-9*3e8);
% disp(['NA = ',num2str(NA),', kz=',num2str(kz),', kx=',num2str(kx)])
